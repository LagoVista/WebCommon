...
$solutionPath = Join-Path $repoRoot 'WebCommon.sln'
...
Write-Host "Restoring WebCommon.sln for package set $Version..."
dotnet restore $solutionPath
if ($LASTEXITCODE -ne 0) { throw "dotnet restore failed with exit code $LASTEXITCODE." }

Write-Host 'Building WebCommon.sln...'
...
$sourceRepository = if ($env:GITHUB_REPOSITORY) { $env:GITHUB_REPOSITORY } else { 'LagoVista/WebCommon' }
